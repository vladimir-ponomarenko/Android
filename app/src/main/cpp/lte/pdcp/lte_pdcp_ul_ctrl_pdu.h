...
static int _decode_lte_pdcp_ul_ctrl_pdu_subpkt(const char* b, int offset, size_t length, json& j) {
    int start = offset;
    int pkt_ver = 0;
    int n_subpkt = 0;
    ...
    if (j.find("Version") != j.end() && j["Version"].is_number()) {
        ...
        LOGD("LTE_PDCP_UL_Ctrl_PDU: Missing or invalid 'Version'");
    ...
    if (j.find("Num Subpkt") != j.end() && j["Num Subpkt"].is_number()) {
        ...
        LOGD("LTE_PDCP_UL_Ctrl_PDU: Missing or invalid 'Num Subpkt'");
        j["error"] = "Missing Num Subpkt in main header";
        return 0;
    }

    LOGD("Decoding LTE_PDCP_UL_Ctrl_PDU with version: %d, num_subpackets: %d", pkt_ver, n_subpkt);
    ...
    switch (pkt_ver) {
        case 1: {
            ...
            for (int i = 0; i < n_subpkt; ++i) {
                ...
                int header_consumed = _decode_by_fmt(LtePdcpUlCtrlPdu_SubpktHeader,
                ...
                if (!header_ok) {
                    LOGD("LTE_PDCP_UL_Ctrl_PDU: Error decoding subpacket %d header or missing fields/invalid size.", i);
                ...
                if (subpkt_id == 194) {
                    if (subpkt_payload_ver == 1) {
                        offset += _decode_by_fmt(LtePdcpUlCtrlPdu_SubpktPayload,
                        ...
                        if (subpkt_j.find("Mode") != subpkt_j.end()) {
                            const char* mode_name = search_name(LtePdcpUlCtrlPdu_Subpkt_Mode, ARRAY_SIZE(LtePdcpUlCtrlPdu_Subpkt_Mode, ValueName), subpkt_j["Mode"]);
                            subpkt_j["Mode"] = (mode_name ? mode_name : "(MI)Unknown");
                        }

                        offset += _decode_by_fmt(LtePdcpUlCtrlPdu_Subpkt_PDU_Header,
                                                 ARRAY_SIZE(LtePdcpUlCtrlPdu_Subpkt_PDU_Header, Fmt),
                        ...
                        subpkt_j["PDCP UL Ctrl PDU"] = pdu_list_j;

                    } else if (subpkt_payload_ver == 24) {
                        offset += _decode_by_fmt(LtePdcpUlCtrlPdu_SubpktPayload_v24,
                        ...
                        offset += _decode_by_fmt(LtePdcpUlCtrlPdu_Subpkt_PDU_Header_v24,
                                                 ARRAY_SIZE(LtePdcpUlCtrlPdu_Subpkt_PDU_Header_v24, Fmt),
                                                 b, offset, length - (offset - start), subpkt_j);
                        ...
                        for (int pdu_idx = 0; pdu_idx < num_PDU; ++pdu_idx) {
                            ...
                            if (pdu_item_j.find("fms") != pdu_item_j.end() && pdu_item_j["fms"].is_number()) {
                                uint16_t fms_raw = pdu_item_j["fms"];
                        ...
                        LOGD("(MI)Unknown LTE PDCP UL Ctrl PDU subpkt version for ID 194: %d", subpkt_payload_ver);
                    ...
                    LOGD("(MI)Unknown LTE PDCP UL Ctrl PDU Subpacket ID: 0x%x", subpkt_id);
                    int remaining_payload_size = subpkt_size - header_consumed;
                    if (remaining_payload_size > 0 && offset + remaining_payload_size <= start + length) {
                        offset += remaining_payload_size;
                    }
        ...
        default:
            LOGD("(MI)Unknown LTE PDCP UL Ctrl PDU packet version: 0x%x", pkt_ver);
            return 0;
    }
}
