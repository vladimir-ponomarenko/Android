...
fun MapScreen(state: MainActivity.MainActivityState, onNavigateTo: (Int) -> Unit) {
    val isDarkTheme = isSystemInDarkTheme() // Проверка темы системы
    val cameraPositionState = rememberCameraPositionState {
        position = CameraPosition.fromLatLngZoom(LatLng(55.0415, 82.9346), 10f)
    }

    // Получаем строку JSON-стиля карты из ресурсов
    val context = LocalContext.current
    val mapStyleJson = context.getString(R.string.style_json) // Загружаем стиль
    ...
    Scaffold(
        ...
        Box(
            ...
            // Вставляем другие элементы UI, например, столбчатые диаграммы
            ...
            Box {
                Button(
                    ...
                    colors = ButtonDefaults.buttonColors(
                        containerColor = if (isDarkTheme) Color(0xFF3C3C3E) else Color(0xFFFFFFFF) // фон кнопки меняется в зависимости от темы
                    ),
                    border = BorderStroke(1.dp, if (isDarkTheme) Color(0x809E9E9E) else Color(0x4D9E9E9E)), // цвет бордера
                    ...
                    Text(
                        ...
                        color = if (isDarkTheme) Color(0xCCFFFFFF) else Color(0xB334204C), // цвет текста
                        fontSize = 16.sp,
                ...
                DropdownMenu(
                    ...
                    modifier = Modifier
                        .width(110.dp)
                        .background(if (isDarkTheme) Color(0xFF3C3C3E) else Color(0xFFFFFFFF)) // Фон меняется в зависимости от темы
...
fun ColorScaleColumn(state: MainActivity.MainActivityState) {
    val isDarkTheme = isSystemInDarkTheme() // Проверка текущей темы
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        Column(
            modifier = Modifier
                ...
                .offset(y = 30.dp)
                .padding(10.dp)
                .background(if (isDarkTheme)  Color(0xFF3C3C3E) else  Color(0xFFFFFFFF), shape = RoundedCornerShape(15.dp)) // Меняем фон в зависимости от темы
...
fun SignalBarChart(
    ...
    Column(
        modifier = Modifier
            .padding(10.dp)
            .background(if (isDarkTheme) Color(0xFF3C3C3E) else  Color(0xFFFFFFFF), shape = RoundedCornerShape(10.dp)) // Фон меняется в зависимости от темы
            .padding(10.dp)
    ) {
        Row(
            modifier = Modifier
            ...
            Text(
                ...
                color = if (isDarkTheme) Color(0xCCFFFFFF) else Color(0xFF34204C), // Цвет текста меняется в зависимости от темы
            ...
            IconButton(
                ...
                Icon(
                    ...
                    modifier = Modifier.size(20.dp),
                    tint = if (isDarkTheme) Color(0xCCFFFFFF) else Color(0xFF34204C) // Меняем цвет иконки в зависимости от темы
                )
            }
        }
                ...
                if (bucketCount > 0 && index < signalValues.size) {
                    ...
                    Row(
                        ...
                        Text(
                            ...
                            fontSize = 10.sp,
                            fontWeight = FontWeight.Normal,
                            color = if (isDarkTheme) Color.White else Color.Black, // Цвет текста меняется в зависимости от темы
                            modifier = Modifier
                        ...
                        Box(
                            ...
                            Text(
                                ...
                                modifier = Modifier
                                    ...
                                    .padding(start = 8.dp),
                                fontSize = 12.sp,
                                color = if (isDarkTheme) Color.Black else Color.White, // Цвет текста меняется в зависимости от темы
                                fontWeight = FontWeight.Bold
                            )
                        }
                    }
